...
While SCD structures may evolve, the 0.5.0 version establishes the following top-level pattern:
...
Tools may introduce additional semantics, but 0.5.0 defines the minimal set.

**Note:** these are SCD-to-SCD relationships. A domain's **Domain Ontology**
(`domain-ontology.md`, RFC-0001) defines a separate, smaller relationship set
(`depends-on`, `relates-to`, `satisfies`) between the domain's `concept:` ids — a
different graph, one level up from the SCDs themselves.
...
Future SCS versions may introduce changes, but SCDs written for 0.5.0 must remain readable by compatible tools.
...
While tooling is out of scope for 0.5.0, the Core Model is designed to support it from the beginning.

---

## 11. Extensibility

SCS 0.5.0 defines the minimal Core Model.
